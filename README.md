# Birdfy Netvue Integration for Home Assistant

[![hacs_badge](https://img.shields.io/badge/HACS-Custom-orange.svg)](https://github.com/hacs/integration)

Home Assistant custom integration for [Birdfy](https://www.birdfy.com/) smart bird feeder cameras using Netvue cloud APIs. Browse and play SD card recordings directly from the HA Media Browser, and download thumbnails or videos to local or network storage.
...
4. Install **Birdfy Netvue Recordings**
5. Restart Home Assistant
6. Go to **Settings → Devices & Services → Add Integration** → search for **Birdfy Netvue**
...
| Entity                      | Description                            |
| --------------------------- | -------------------------------------- |
| `sensor.birdfy_netvue_last_event` | Label and attributes of the last detection |
| `camera.birdfy_netvue_thumbnail`  | Thumbnail image of the last detection  |
...
Open the **Media Browser** in the HA sidebar → **Birdfy Netvue** → select a day → play a clip.
...
### `birdfy_netvue.download_thumbnails`
...
| `path`    | Full path where thumbnails will be saved                                 | `/media/birdfy_netvue_thumbnails` |

Example automation:

```yaml
action:
  - service: birdfy_netvue.download_thumbnails
    data:
      days: 1
      share: my_nas
```
...
| `days`    | Number of days to fetch (1–30)                                           | `1`                  |
| `share`   | Network share name mounted in HA under `/media/` (takes priority over `path`) | —               |
| `path`    | Full path where videos will be saved                                     | `/media/birdfy_netvue_videos` |

Example automation:

```yaml
action:
  - service: birdfy_netvue.download_videos
    data:
      days: 1
      share: my_nas
```
...
- This fork installs as the `birdfy_netvue` domain so it can be tested alongside other integrations that use the `birdfy` domain
